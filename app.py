...
<div class="main-header">
  <div style="display:flex;justify-content:space-between;align-items:center">
    <div>
      ...
      <div class="main-subtitle">Yahoo Finance + Finviz · 15-min delayed · Auto-refresh every 15 min · <b style=\"color:#27ae60\">✅ v3.2 — Setup Scanner with filters</b></div>
    </div>
...
with tab_s2:
    ...
    with col2:
        ...
        with c_btn2:
            if st.button("🗑️ Clear & Reset", help="Clear cached results and start fresh"):
                st.session_state.stage2_results    = None
                st.session_state.stage2_timestamp  = None
                st.session_state.setup_scan_results = None
                st.cache_data.clear()
...
with tab_scan:
    ...
    if st.session_state.stage2_results is None:
        ...
        # ── Scan selection (always visible) ──────────────────────────────
        st.markdown(
            "<div style='background:#0d1520;border:1px solid #1e3a5f;border-radius:8px;"
            "padding:14px 18px;margin-bottom:16px'>"
            "<div style='color:#e8f4fd;font-weight:700;font-size:13px;margin-bottom:12px'>"
            "🎛️ Step 1 — Select which scans to run:</div>",
            unsafe_allow_html=True
        )
        fc1, fc2, fc3 = st.columns(3)
        with fc1:
            use_s1 = st.checkbox("🔵 Scan 1 — Low Volatility",  value=True, key="pre_s1")
            st.caption("Both last 2 days range < threshold")
        with fc2:
            use_s2 = st.checkbox("🟡 Scan 2 — Pivot Area",      value=True, key="pre_s2")
            st.caption("Last 2 highs within threshold %")
        with fc3:
            use_s3 = st.checkbox("🟣 Scan 3 — Inside Day",      value=True, key="pre_s3")
            st.caption("Today's range inside yesterday's")
        ...
        if st.button("🔍 Run Setup Scans", type="primary", key="run_setup"):
            ...
            for i, ticker in enumerate(tickers):
                ...
                if r:
                    # Apply pre-scan filter — only keep if passes a selected scan
                    if not ((use_s1 and r["scan1"]) or (use_s2 and r["scan2"]) or (use_s3 and r["scan3"])):
                        continue
        ...
        if st.session_state.setup_scan_results:
            ...
            # ── Post-scan filter: AND/OR toggle ───────────────────────────
            st.markdown(
                "<div style='color:#e8f4fd;font-weight:700;font-size:13px;"
                "margin-bottom:8px'>🎛️ Step 2 — Filter results:</div>",
                unsafe_allow_html=True
            )
            pf1, pf2, pf3, pf4, pf5 = st.columns([2,2,2,2,3])
            with pf1:
                show_s1 = st.checkbox("🔵 Low Vol",   value=use_s1, key="f_s1", disabled=not use_s1)
            with pf2:
                show_s2 = st.checkbox("🟡 Pivot",     value=use_s2, key="f_s2", disabled=not use_s2)
            with pf3:
                show_s3 = st.checkbox("🟣 Inside Day",value=use_s3, key="f_s3", disabled=not use_s3)
            with pf4:
                logic = st.radio("Combine with",
                                 ["OR — any ticked", "AND — all ticked"],
            ...
            with pf5:
                st.markdown(
                    ...
                    "<b>OR</b> = show if it passes <i>any</i> ticked scan<br>"
                    "<b>AND</b> = show only if it passes <i>all</i> ticked scans<br>"
                    "<span style='color:#4a5568'>Untick to exclude a scan entirely</span>"
                    "</div>",
                    unsafe_allow_html=True
                )
            ...
            selected = []
            if show_s1: selected.append("scan1")
            if show_s2: selected.append("scan2")
            if show_s3: selected.append("scan3")

            if not selected:
                st.warning("Tick at least one scan above.")
            else:
                if use_and:
